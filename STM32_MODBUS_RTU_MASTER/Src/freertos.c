...
//			USER_USART3_UART_Init();
			...
			uiSysState++;
			break;
		case SYS_MB_PROTOCOL: // Run this task to init Modbus stack
			...
			break;
		case SYS_MB_APP: // Run Modbus Uplink Task
			...
			break;
		case SYS_MB_DOWNLINK: // Run Modbus Downlink Task
		...
		case SYS_NET_TIME: // Get time from internet
...
//					printf("\r\n Killing netTimeTask module: OK \r\n");
			...
			break;
		case SYS_CORE_DISCOV: // Not complete
		...
		case SYS_HTTP: // Not complete
			...
			break;
		case SYS_RECORD: // Check RECORD file in SDCARD. if it is, then send those to server
			printf("\r\n SYS_RECORD: Starting...  \r\n");
			xQueueMbMqtt_t xQueueMbMqtt;
			...
			uiSysState++;
			break;
		case SYS_DEVICE: // Load device from SD card -> Allocate memory to contain those
...
//				}
				...
				uiSysState++;
			break;
		case SYS_DEFAULT: // Check command state to execute specific command
			if (gotCommandFlag == 1) {
				gotCommandFlag = 0;
